...
> specific repo, not in the abstract — if the rule is violated.
...
group ever got configured automatically — a silent partial failure, not an
error. Worse: Terraform's `local-exec` has no retry semantics matched to
Ansible's — a transient SSH failure during `ansible-playbook` would either
...
easily re-trigger `ansible-playbook` runs that fight with in-place config
changes (recall `sabnzbd.yml`/`bazarr.yml`'s `force: false` templating —
designed around Ansible controlling its own idempotent re-run cadence, not
Terraform's).
...
(e.g., by tainting and re-applying just the template resource), you'd end up
with two templates on disk, one tagged `default` and reachable by the tag
...
entirely — you accept that `application` will need a subsequent `apply` once
Ansible catches up, rather than have one giant `apply` fail to converge.
...
short term — but the failure mode this avoids is worse than a "break": if
`root`'s password ever needed rotating or leaked (see [[secrets]] for how
...
the alias pattern, the answer is: `grep -rn "proxmox.root" workspace/` and
you have the complete list. `modules/proxmox_vm/main.tf:45` (the VM clone
resource) is currently the only place inside the module itself.

## Let `pcie_devices` drive the machine type — don't set it manually
...
**What would break if violated**: precisely the class of bug that's common
in hand-rolled Proxmox Terraform configs elsewhere — a VM that boots, has no
visible boot failure, but silently fails PCI passthrough because the BIOS
type doesn't support it, with the failure surfacing much later as "GPU
...
actually always correlated (`rombar` following `pcie`) — a caution that the
"one variable, multiple effects" pattern is a good idea specifically when
the effects genuinely always travel together, and a footgun when they don't.
...
correctly, rather than a clean "nothing happened, fix the input and re-run."
...
- [ ] Give a concrete scenario where `observability_control` and
      ...
      port — and explain why the prefix + group_vars re-export pattern
      supports that without restructuring.
