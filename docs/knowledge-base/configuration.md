...
decrypted via a `community.sops.sops` lookup with a jq-style `extract` path.
**Even the inventory file that queries Proxmox contains no plaintext
credential.**
...
so this is intentional layering, not a conflict. Two sources describe the
same `pve` host.
...
> file this vault cannot read. "No Ansible edit needed to add a workload
> host" holds for observability membership. For workload-group membership
> like `media_platform`, it is **unverified**, possibly false. Scaling past
> one media host requires checking `11_media_platform.ini` first.
...
**every single run**, regardless of `--limit`, unless its `sops`/`secrets`
tags are explicitly excluded. It installs the `sops` CLI binary on the
control node via `community.sops.install` (`ansible/sops.yml`). This is a
control-node concern, not a managed-host concern, distinct from what the
playbook name ("installs SOPS on every host") might suggest.
...
so a queued container-restart handler still fires even if a later task in
the same play fails. **`media_platform.yml` does not set `force_handlers`**.
A media-stack config change may not take effect after a partially-failed run
...
task-level inside each playbook. This is what lets
...
role input against it **before the first task runs**, so a malformed
variable fails at role entry with a typed error, not partway through a play
...
same data structure can tear down storage as easily as create it: setting
the states to `absent` and re-running does so.
...
> overrides it. It does nothing in that case, because looping over an empty
> list is a no-op in every task file.
...
- `sabnzbd.yml` and `bazarr.yml` both template their config with **`force:
  ...
  playbook.
- `configarr.yml`'s templated file gets mode `0440`, read-only with no write
  even for the owning user, consistent with Configarr's `!env` indirection
  (see [[gpu-passthrough]] and below): the file never needs in-place edits
  because secrets are pulled from the container's environment at runtime,
...
installed kernel modules would not be loaded yet when the containers start.
...
Dashboards are version-controlled, not clicked into existence and lost on a
container rebuild. See [[observability]] for the full topology, the
Prometheus file-based service discovery mechanism, and several template
defects found while reading these roles.
...
pattern: it inherits everything from `observability` and then flips on
exactly three booleans for the PVE host alone
...
over parent), but the *pattern*, unprefixed shared value to prefixed
role-interface value, one hop apart, is the repo's own convention, not
something Ansible provides by default.
...
default for a new role rather than an addition made later.
